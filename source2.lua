...
Ultrastellar.__index = Ultrastellar

local selectedTab

Ultrastellar._promptExists = false
Ultrastellar._colorPickerExists = false

local GlobalTweenInfo = TweenInfo.new(0.2, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut)

function Ultrastellar:set_defaults(defaults, options)
...
function Ultrastellar:change_theme(toTheme)
	Ultrastellar.CurrentTheme = toTheme
	local c = self:lighten(toTheme.Tertiary, 20)
	Ultrastellar.DisplayName.Text = "Welcome, <font color='rgb(" ..  math.floor(c.R*255) .. "," .. math.floor(c.G*255) .. "," .. math.floor(c.B*255) .. ")'> <b>" .. LocalPlayer.DisplayName .. "</b> </font>"
	for color, objects in next, Ultrastellar.ThemeObjects do
		local themeColor = Ultrastellar.CurrentTheme[color]
		for _, obj in next, objects do
			local element, property, theme, colorAlter = obj[1], obj[2], obj[3], obj[4] or 0
			local themeColor = Ultrastellar.CurrentTheme[theme]
			local modifiedColor = themeColor
			if colorAlter < 0 then
				modifiedColor = Ultrastellar:darken(themeColor, -1 * colorAlter)
			elseif colorAlter > 0 then
				modifiedColor = Ultrastellar:lighten(themeColor, colorAlter)
			end
...
function Ultrastellar:object(class, properties)
	...
	function methods:tween(options, callback)
		local options = Ultrastellar:set_defaults({
			Length = 0.2,
	...
	function methods:object(class, properties)
		...
		return Ultrastellar:object(class, properties)
	end
	...
	function methods:stroke(color, thickness, strokeMode)
		...
		if type(color) == "table" then
			...
			table.insert(Ultrastellar.ThemeObjects[theme], {stroke, "Color", theme, colorAlter})
		elseif type(color) == "string" then
			local themeColor = Ultrastellar.CurrentTheme[color]
			stroke.Color = themeColor
			table.insert(Ultrastellar.ThemeObjects[color], {stroke, "Color", color, 0})
		else
			stroke.Color = color
		end
		...
		Theme = function(value)
			for property, obj in next, value do
				if type(obj) == "table" then
					...
					local themeColor = Ultrastellar.CurrentTheme[obj]
...
function Ultrastellar:show(state)
	self.Toggled = state
...
function Ultrastellar:darken(color, f)
	local h, s, v = Color3.toHSV(color)
	f = 1 - ((f or 15) / 80)
	return Color3.fromHSV(h, math.clamp(s/f, 0, 1), math.clamp(v*f, 0, 1))
end

function Ultrastellar:lighten(color, f)
	local h, s, v = Color3.toHSV(color)
	f = 1 - ((f or 15) / 80)
	return Color3.fromHSV(h, math.clamp(s*f, 0, 1), math.clamp(v/f, 0, 1))
end
...
function Ultrastellar:set_status(txt)
	self.statusText.Text = txt
end

function Ultrastellar:create(options)
	...
	if readfile and writefile and isfile then
		if not isfile("UltrastellarSettings.json") then
			writefile("UltrastellarSettings.json", HTTPService:JSONEncode(settings))
		end
		settings = HTTPService:JSONDecode(readfile("UltrastellarSettings.json"))
		Ultrastellar.CurrentTheme = Ultrastellar.Themes[settings.Theme]
	...
	if getgenv and getgenv().UltrastellarUI then
		getgenv():UltrastellarUI()
		getgenv().UltrastellarUI = nil
	end
				...
				local Input = core.InputBegan:connect(function(Key)
					if Key.UserInputType == Enum.UserInputType.MouseButton1 then
						...
						while RunService.RenderStepped:wait() and UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) do
							...
							if Ultrastellar.LockDragging then
	...
	closeButton.MouseLeave:connect(function()
		closeButton:tween{ImageColor3 = Ultrastellar.CurrentTheme.StrongText}
	end)
	...
	if getgenv then
		getgenv().UltrastellarUI = closeUI
	end
	...
	Ultrastellar.UrlLabel = link
	Ultrastellar.Url = options.Link
	...
	do
		...
		homeButton.MouseButton1Click:Connect(function()
			...
			Ultrastellar.UrlLabel.Text = Ultrastellar.Url .. "/home"
		end)
	end
	...
	local displayName; do
		...
		Ultrastellar.DisplayName = displayName
	end
	...
	local settingsTab = Ultrastellar.tab(mt, {
		Name = "Settings",
	...
	settingsTab:keybind{
		...
		Callback = function()
			...
			Ultrastellar:show(self.Toggled)
		end,
	}
	...
	settingsTab:toggle{
		...
		Callback = function(state)
			Ultrastellar.LockDragging = state
		end,
	}
	...
	settingsTab:slider{
		...
		Callback = function(value)
			Ultrastellar.DragSpeed = (20 - value)/100
		end,
	}

	local creditsTab = Ultrastellar.tab(mt, {
		Name = "Credits",
	...
	creditsTab:credit{Name = "Abstract", Description = "UI Ultrastellar Developer", Discord = "Abstract#8007", V3rmillion = "AbstractPoo"}
	creditsTab:credit{Name = "Deity", Description = "UI Ultrastellar Developer", Discord = "Deity#0228", V3rmillion = "0xDEITY"}
	creditsTab:credit{Name = "Repository", Description = "UI Ultrastellar Repository", Github="https://github.com/deeeity/Ultrastellar-lib/blob/master/src.lua"}

	return mt
end

function Ultrastellar:notification(options)
	options = self:set_defaults({
		Title = "Notification",
...
function Ultrastellar:tab(options)
	options = self:set_defaults({
		Name = "New Tab",
	...
	local tabButton = Ultrastellar:object("TextButton", {
		BackgroundTransparency = 1,
	...
	do
		...
		tabButton.MouseButton1Click:Connect(function()
			...
			Ultrastellar.UrlLabel.Text = Ultrastellar.Url .. "/" .. options.Name:lower()
		end)

		quickAccessButton.MouseEnter:connect(function()
			quickAccessButton:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Tertiary}
		end)

		quickAccessButton.MouseLeave:connect(function()
			quickAccessButton:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Secondary}
		end)
		...
		quickAccessButton.MouseButton1Click:connect(function()
			if not tabButton.Visible then
				...
				tabButton:fade(false, Ultrastellar.CurrentTheme.Main, 0.1)			
	...
	tabButtonClose.MouseButton1Click:connect(function()
		tabButton:fade(true, Ultrastellar.CurrentTheme.Main, 0.1)
		...
		elseif tabButton == lastTab[2] then
			...
			Ultrastellar.UrlLabel.Text = Ultrastellar.Url .. "/" .. lastTab[3]:lower()
		else
			tab.Visible = false
			lastTab[2]:tween{BackgroundTransparency = 0.15}
			lastTab[1].Visible = true
...
function Ultrastellar:_resize_tab()
...
function Ultrastellar:toggle(options)
	options = self:set_defaults({
		Name = "Toggle",
		StartingState = false,
		Description = nil,
	...
	do
		...
		toggleContainer.MouseEnter:connect(function()
			hovered = true
			toggleContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 10)}
		end)

		toggleContainer.MouseLeave:connect(function()
			hovered = false
			if not down then
				toggleContainer:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Secondary}
			end
		end)

		toggleContainer.MouseButton1Down:connect(function()
			toggleContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 20)}
		end)

		UserInputService.InputEnded:connect(function(key)
			if key.UserInputType == Enum.UserInputType.MouseButton1 then
				toggleContainer:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Secondary)) or Ultrastellar.CurrentTheme.Secondary}
			end
		end)
...
function Ultrastellar:dropdown(options)
	options = self:set_defaults({
		Name = "Dropdown",
		StartingText = "Select...",
		Items = {},
	...
	for i, item in next, items do
		...
		do
			...
			newItem.MouseEnter:connect(function()
				hovered = true
				newItem:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Tertiary}
			end)

			newItem.MouseLeave:connect(function()
				hovered = false
				if not down then
					newItem:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 25)}
				end
			end)

			newItem.MouseButton1Down:connect(function()
				newItem:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Tertiary, 10)}
			end)

			UserInputService.InputEnded:connect(function(key)
				if key.UserInputType == Enum.UserInputType.MouseButton1 then
					newItem:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Tertiary, 5)) or self:lighten(Ultrastellar.CurrentTheme.Secondary, 25)}
				end
			end)
	...
	do
		...
		dropdownContainer.MouseEnter:connect(function()
			hovered = true
			dropdownContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 10)}
		end)

		dropdownContainer.MouseLeave:connect(function()
			hovered = false
			if not down then
				dropdownContainer:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Secondary}
			end
		end)

		dropdownContainer.MouseButton1Down:connect(function()
			dropdownContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 20)}
		end)

		UserInputService.InputEnded:connect(function(key)
			if key.UserInputType == Enum.UserInputType.MouseButton1 then
				dropdownContainer:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Secondary)) or Ultrastellar.CurrentTheme.Secondary}
			end
		end)
		...
		for i, item in next, items do
			...
			do
				...
				newItem.MouseLeave:connect(function()
					hovered = false
					if not down then
						newItem:tween{BackgroundColor3 = Ultrastellar:lighten(Ultrastellar.CurrentTheme.Secondary, 25)}
					end
				end)

				newItem.MouseButton1Down:connect(function()
					newItem:tween{BackgroundColor3 = Ultrastellar:lighten(Ultrastellar.CurrentTheme.Tertiary, 10)}
				end)

				UserInputService.InputEnded:connect(function(key)
					if key.UserInputType == Enum.UserInputType.MouseButton1 then
						newItem:tween{BackgroundColor3 = (hovered and Ultrastellar:lighten(Ultrastellar.CurrentTheme.Tertiary, 5)) or Ultrastellar:lighten(Ultrastellar.CurrentTheme.Secondary, 25)}
					end
				end)
...
function Ultrastellar:section(options)
	options = self:set_defaults({
		Name = "Section"
	}, options)
...
function Ultrastellar:button(options)
	options = self:set_defaults({
		Name = "Button",
		Description = nil,
		Callback = function() end
	...
	do
		...
		buttonContainer.MouseEnter:connect(function()
			hovered = true
			buttonContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 10)}
		end)

		buttonContainer.MouseLeave:connect(function()
			hovered = false
			if not down then
				buttonContainer:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Secondary}
			end
		end)

		buttonContainer.MouseButton1Down:connect(function()
			buttonContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 20)}
		end)

		UserInputService.InputEnded:connect(function(key)
			if key.UserInputType == Enum.UserInputType.MouseButton1 then
				buttonContainer:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Secondary)) or Ultrastellar.CurrentTheme.Secondary}
			end
		end)
...
function Ultrastellar:color_picker(options)
	options = self:set_defaults({
		Name = "Color Picker",
		Description = nil,
		Style = Ultrastellar.ColorPickerStyles.Legacy,
		Followup = false,
	...
	local icon = buttonContainer:object("ImageLabel", {
		...
		ImageColor3 = Ultrastellar.CurrentTheme.Tertiary
	})
	...
	do
		...
		buttonContainer.MouseButton1Click:connect(function()
			if Ultrastellar._colorPickerExists then return end
			Ultrastellar._colorPickerExists = true
			local hue, sat, val;
				...
				do
					...
					do
						...
						button.MouseEnter:connect(function()
							hovered = true
							button:tween{BackgroundTransparency = 0, TextColor3 = self:lighten(Ultrastellar.CurrentTheme.StrongText, 15)}
						end)

						button.MouseLeave:connect(function()
							hovered = false
							if not down then
								button:tween{BackgroundTransparency = 1, TextColor3 = self:darken(Ultrastellar.CurrentTheme.Tertiary, 10)}
							end
						end)

						button.MouseButton1Down:connect(function()
							button:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Tertiary, 20)}
						end)
						...
						UserInputService.InputEnded:connect(function(key)
							if key.UserInputType == Enum.UserInputType.MouseButton1 then
								...
								if hovered then
									button:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Tertiary}
								end
							end
						end)
				...
				do
					...
					do
						...
						pickBtn.MouseEnter:connect(function()
							hovered = true
							pickBtn:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Tertiary, 10)}
						end)

						pickBtn.MouseLeave:connect(function()
							hovered = false
							if not down then
								pickBtn:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Tertiary}
							end
						end)

						pickBtn.MouseButton1Down:connect(function()
							pickBtn:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Tertiary, 20)}
						end)

						UserInputService.InputEnded:connect(function(key)
							if key.UserInputType == Enum.UserInputType.MouseButton1 then
								pickBtn:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Tertiary)) or Ultrastellar.CurrentTheme.Tertiary}
							end
						end)
...
function Ultrastellar:credit(options)
	options = self:set_defaults({
		Name = "Creditor",
		Description = nil
	}, options)
...
function Ultrastellar:_theme_selector()

	local themesCount = 0

	for _ in next, Ultrastellar.Themes do
		themesCount += 1
	end
	...
	for themeName, themeColors in next, Ultrastellar.Themes do
		local count = 0
		...
		if count >= 5 then
			...
			theme.MouseButton1Click:connect(function()
				Ultrastellar:change_theme(Ultrastellar.Themes[themeName])
...
function Ultrastellar:keybind(options)
	options = self:set_defaults({
		Name = "Keybind",
		Keybind = nil,
		Description = nil,
	...
	do
		...
		keybindContainer.MouseEnter:connect(function()
			hovered = true
			keybindContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 10)}
		end)

		keybindContainer.MouseLeave:connect(function()
			hovered = false
			if not down then
				keybindContainer:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Secondary}
			end
		end)

		keybindContainer.MouseButton1Down:connect(function()
			keybindContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 20)}
		end)

		UserInputService.InputEnded:connect(function(key)
			if key.UserInputType == Enum.UserInputType.MouseButton1 then
				keybindContainer:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Secondary)) or Ultrastellar.CurrentTheme.Secondary}
			end
		end)
...
function Ultrastellar:prompt(options)
	options = self:set_defaults({
		Followup = false,
		Title = "Prompt",
		Text = "yo momma dead",
	...
	if Ultrastellar._promptExists and not options.Followup then return end
	Ultrastellar._promptExists = true
	...
	for text, callback in next, options.Buttons do
		...
		do
			...
			button.MouseEnter:connect(function()
				hovered = true
				button:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Tertiary, 10)}
			end)
			...
			UserInputService.InputEnded:connect(function(key)
				if key.UserInputType == Enum.UserInputType.MouseButton1 then
					button:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Tertiary)) or Ultrastellar.CurrentTheme.Tertiary}
				end
			end)
...
function Ultrastellar:cp(options)
	return Ultrastellar.color_picker(self, options)
end
function Ultrastellar:colorpicker(options)
	return Ultrastellar.color_picker(self, options)
end

function Ultrastellar:slider(options)
	options = self:set_defaults({
		Name = "Slider",
		Default = 50,
		Min = 0,
	...
	do
		...
		sliderContainer.MouseEnter:connect(function()
			hovered = true
			sliderContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 10)}
		end)

		sliderContainer.MouseLeave:connect(function()
			hovered = false
			if not down then
				sliderContainer:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Secondary}
			end
		end)
		...
		UserInputService.InputEnded:connect(function(key)
			if key.UserInputType == Enum.UserInputType.MouseButton1 then
				down = false
				sliderContainer:tween{BackgroundColor3 = (hovered and self:lighten(Ultrastellar.CurrentTheme.Secondary)) or Ultrastellar.CurrentTheme.Secondary}
			end
		end)

		sliderContainer.MouseButton1Down:connect(function()
			sliderContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 20)}
			down = true
...
function Ultrastellar:textbox(options)
	options = self:set_defaults({
		Name = "Text Box",
	...
	do
		...
		textboxContainer.MouseEnter:connect(function()
			textboxContainer:tween{BackgroundColor3 = self:lighten(Ultrastellar.CurrentTheme.Secondary, 10)}
		end)

		textboxContainer.MouseLeave:connect(function()
			hovered = false
			if not down then
				textboxContainer:tween{BackgroundColor3 = Ultrastellar.CurrentTheme.Secondary}
			end
		end)
...
function Ultrastellar:label(options)
...
return setmetatable(Ultrastellar, {
	__index = function(_, i)
		return rawget(Ultrastellar, i:lower())
	end
})
